    ...
    : QDialog(parent, f)
{
    ...
    if (mode == Debug) {
        if (problemList.isEmpty()) {
            message = i18n(
                "soundKonverter couldn't find any missing packages.\nIf you are missing some file formats you might need to install an additional plugin via "
        ...
        if (problemList.isEmpty()) {
            message = i18n(
                "Ripping audio CDs is not supported by any installed plugin.\nPlease have a look at your distributions package manager in order to get a cd "
                "ripper plugin for soundKonverter.");
        } else {
